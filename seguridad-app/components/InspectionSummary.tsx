...
import { CheckCircle, AlertTriangle, Send, FileDown, ArrowLeft, PenTool } from 'lucide-react';
...
export const InspectionSummary: React.FC<Props> = ({ log, onConfirm, onBack }) => {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      // If content is longer than one page, simple approach: split it or shrink it?
      // Since html2canvas takes a screenshot, a very long report might get cut.
      // For a robust multi-page implementation in browser we normally iterate.
      // Here we will fit it by scaling if possible or adding pages.
    ...
    <div className="space-y-6">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6">
        <div className="text-center py-6">
          ...
          <p className="text-slate-500">Revise y genere el informe final.</p>
        </div>
        ...
        <div className="fixed bottom-0 left-0 w-full p-4 bg-white border-t border-slate-200 flex gap-3 pb-8 z-20">
          <button 
            onClick={onBack}
            className="flex-1 py-3 px-4 rounded-xl border border-slate-300 text-slate-600 font-bold"
          >
            Volver
          </button>
          <button 
            ...
            className="flex-[2] py-3 px-4 rounded-xl bg-safety-600 text-white font-bold shadow-lg shadow-safety-200 hover:bg-safety-700 flex items-center justify-center gap-2 disabled:opacity-70"
          >
            {isGenerating ? 'Generando...' : (
               <>
                <FileDown className="w-5 h-5" /> Guardar y Enviar
               </>
            )}
          </button>
        </div>
      ...
      <div className="absolute top-0 left-0 w-full z-[-1] overflow-hidden h-0">
        <div ref={reportRef} className="bg-white p-8 w-[210mm] min-h-[297mm] mx-auto text-slate-900 font-sans">
